...
function createNewCard(data) { //функция для вызова класса с нужными пораметрами
  const newCard = new Card(data, '.cards-template', handleCardClick); // новый элемент в который засовываю элемент с именем и ссылкой, селектор для копирования разметки, и функцию слушателя нажатия на картинку.
  const cardElement = newCard.generateCard(); //в переменную генерирую нужную новую карточку с значениями.
  return cardElement; //возвращаю на страницу
};
